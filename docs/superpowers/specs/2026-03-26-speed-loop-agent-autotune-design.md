...
v1 的核心原则仍然是“结构化指标优先”，但原始波形的定位调整为：

- 不直接让 Agent 把整段原始波形当作黑盒优化目标
- 允许 worker 产出有限的波形摘要或波形文件路径
- 在结构化指标互相矛盾、疑似测量噪声、低速纹波和真实闭环恶化难以区分时，允许 Agent 把波形作为辅助证据
...
- 本版本不以原始波形作为主要优化目标，仍然优先使用结构化指标；但允许把原始波形作为二级参考，用于异常复核、结构化指标解释冲突时的辅助判断，以及批末生成更可信的建议。
...
4. **状态与历史层**
   ...
   - `logs/agent_waveforms/`：可选的波形快照或裁剪后的原始序列
...
`waveform_path` 和 `waveform_digest` 的定位是：

- `waveform_path`：指向本轮保存的原始或裁剪波形数据文件
- `waveform_digest`：由 worker 预先提炼的轻量摘要，例如关键段峰值、抖动区间、停止收尾特征、异常片段索引

Agent 在常规轮次中优先依赖结构化指标，仅在以下场景读取波形相关信息：

- 结构化指标互相冲突
- 判断是低速固有纹波还是闭环振荡存在歧义
- `dominant_issue` 为 `noisy_measurement` 或 `saturation_limited`
- 批末需要提升建议可信度
...
- Agent 不能仅凭波形视觉印象推翻结构化评分排序
- 波形只能作为“解释、复核、降置信度或触发保守策略”的证据
- 若波形判断与结构化指标明显冲突，默认优先保守，不自动做激进放大调整
...
- 当结构化指标不足以区分“天然纹波”和“真实闭环异常”时，允许辅以波形复核，但结论必须回落到可解释的结构化判断上
...
- 结构化指标与波形提示冲突时，Agent 采取保守策略而不是激进放大
