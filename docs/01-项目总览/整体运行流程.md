...
TM0_IRQHandler
  -> run_time_1
       ...
       -> pid_steer_update
       -> a_run_track_element_update_gate(speed, angle_target)
       -> pid_angle_update
...
- `a_run_track_element_update_gate(int *speed, float *angle_target)`：推进特殊元素仲裁，决定当前开放左圆环、右圆环、圆桶、跷跷板还是墙面流程；跷跷板阶段会覆盖速度，完成后继续执行阶梯增速，圆环阶段会覆盖目标角速度。
...
特殊元素仲裁在 `a_run_track_element_update_gate(int *speed, float *angle_target)` 中执行，并跟随 5ms 主控制链更新。当前顺序是串行开放：
...
大圆环（3 左 / 4 右）与大小圆环共用同一状态机，在元素序列中单独填编号即可，
仲裁层把大圆环参数组 `app.ring.large_profile` 的指针传给共用状态机（菜单 `RING→LARGE`）。
...
- 圆环过程中按状态覆盖目标角速度：ENTRY 阶段通过 `a_run_ring_update_angle_target()` 强制零角速度直走，环内转向由电感偏置（`ring_data.angle_set` 保留固定角速度覆盖能力，当前未使用）驱动差比和完成。
- 左/右环完成事件只消费一次，随后进入圆桶流程。
- 圆桶流程通过电感强信号窗口、回地确认和稳定延时推进。
- 跷跷板/飞坡是否进入由元素序列决定；需要跳过时从序列中删掉 7。
