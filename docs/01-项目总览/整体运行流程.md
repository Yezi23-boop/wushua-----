...
- `a_run_track_element_update_gate()`：推进特殊元素仲裁，决定当前开放左圆环、圆桶、跷跷板还是墙面流程。
- `a_run_track_element_update_integrals()`：为圆环流程累计里程和 gyro_z 转角。
...
左圆环
  -> 圆桶
  -> 跷跷板/飞坡（当 app.fly.fly_ramp_enable == 1）
  -> 墙面
  -> 回到左圆环
```
...
- 左环完成事件只消费一次，随后进入圆桶流程。
- 圆桶流程通过电感强信号窗口、回地确认和稳定延时推进。
- 若 `app.fly.fly_ramp_enable == 1`，圆桶后进入跷跷板/飞坡流程；否则直接进入墙面流程。
