...
- `control_init()`：初始化左右速度环、电感误差环和圆环专用角速度环。随后调用 `control_apply_config()`，把 EEPROM 恢复到 `app` 的配置同步到 `PID` 运行实例。
...
TM0_IRQHandler
  -> run_time_1
       ...
       -> pid_steer_update
       -> a_run_track_element_update_gate(speed, ring_angle_target)
       -> pid_angle_update（仅圆环目标非零时）
       -> pid_speed_update(left/right)
       -> left/right speed PID output ∓ steer_pwm
       -> motor_output
```
...
- `imu_update_gyro_z_from_imu660rc()`：刷新当前 2ms 控制周期使用的 `gyro_z`，供圆环转角积分使用。
- `pid_steer_update()`：根据电感偏差 `Err` 生成最终差速 PWM。当前通过 `steer_div_10` 分频，每 3 拍约 6ms 更新一次。
- `a_run_track_element_update_gate(float *speed, float *angle_target)`：推进特殊元素仲裁；跷跷板阶段会覆盖速度，圆环固定转向阶段会给出目标角速度。
- `pid_angle_update()`：仅在圆环目标角速度非零时启用，输出圆环差速 PWM；普通巡线不经过该环。
- `pid_speed_update()`：左右轮分别以同一 `speed_active` 为目标闭环，输出各自基础 PWM。
- PWM 合成：左轮为左速度环输出减差速 PWM，右轮为右速度环输出加差速 PWM。
...
特殊元素仲裁在 `a_run_track_element_update_gate(float *speed, float *angle_target)` 中执行，并跟随 2ms 主控制链更新。当前顺序是串行开放：
...
- 圆环过程中会用 `ring_data.diff_set` 覆盖目标角速度，非零时启用圆环角速度环。
- 左/右环完成事件只消费一次，随后进入圆桶流程。
- 圆桶流程通过电感强信号窗口、回地确认和稳定延时推进。
- 跷跷板/飞坡是否进入由元素序列决定；需要跳过时从序列中删掉 5。
- 跷跷板/飞坡流程由 `a_run_fly_update_speed()` 处理弱磁检测、离线保持、落地恢复和完成事件；完成后的 COOLDOWN 由 `a_run_fly_update_release_speed()` 继续阶梯增速，不要求下一个元素是墙面。
...
| --- | --- | --- |
| `app` | `eeprom.c` | 全局配置真值 |
| `PID` | `pid.c` | 左右速度环、电感误差环及圆环专用角速度环状态 |
