...
```text
1 左圆环 -> 5 圆桶 -> 7 跷跷板/飞坡 -> 6 墙面 -> 1 左圆环
```
...
| 3 | 大圆环左 | 可执行，复用圆环状态机，参数取大圆环组 |
| 4 | 大圆环右 | 可执行，复用圆环状态机并反向控制，参数取大圆环组 |
| 5 | 圆桶 | 可执行 |
| 6 | 墙面 | 可执行 |
| 7 | 跷跷板/飞坡 | 可执行，是否进入由元素序列决定 |
| 8 | 双十字 | 可执行 |
| 9 | 单十字 | 可执行，入口判定与双十字相同 |

如果关闭 `app.start.element_enable`，模块会清理圆环、圆桶、墙面和飞坡状态，并对外显示为空元素；重新开启后的第一拍才会扫描 `E1~E8`，进入序列中的第一个有效元素。全空、全非法或当前不可执行时，运行期会进入空状态，不会改写 `app.start.element_seq[]` 或 EEPROM。

运行中修改序列不会打断当前元素，当前元素完成后才按新序列跳转。填入右圆环编号 `2` 时会复用圆环状态机并反向控制入口与目标角速度。填入大圆环编号 `3/4` 时同样复用圆环状态机：仲裁层把大圆环参数组 `&app.ring.large_profile` 作为指针传入，入口确认时锁存，菜单路径为 `RING→LARGE`。

## 各阶段职责

- 左圆环/右圆环：入口要求四路电感满足特征，并使用连续确认和上升沿约束；环内转向由电感偏置驱动差比和，`ring_data.angle_set` 保留固定角速度覆盖能力（当前未使用）。
- 大圆环左/右：状态机与大小圆环完全一致（含入口五路电感阈值），仅参数组独立；入口识别阈值暂与小圆环共用，若现场大环特征差异明显再单独参数化。
