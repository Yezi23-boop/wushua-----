...
#if IMU660RA_USE_SOFT_IIC         // 软件 IIC 驱动接口定义
//====================================================软件 IIC 驱动====================================================
#define IMU660RA_SOFT_IIC_DELAY (1) // 软件 IIC 延时参数 值越小 IIC 通讯速率越快
...
#define IMU660RA_SPC_PIN SPI1_CH1_SCLK_P17            // 硬件 SPI SCK 引脚
#define IMU660RA_SDI_PIN SPI1_CH1_MOSI_P15            // 硬件 SPI MOSI 引脚
#define IMU660RA_SDO_PIN SPI1_CH1_MISO_P16            // 硬件 SPI MISO 引脚
//====================================================硬件 SPI 驱动====================================================
#endif
#define IMU660RA_CS_PIN (IO_P47) // CS 片选管脚
#define IMU660RA_CS(x) ((x) ? (gpio_high(IMU660RA_CS_PIN)) : (gpio_low(IMU660RA_CS_PIN)))

typedef enum
{
    IMU660RA_ACC_SAMPLE_SGN_2G,  // 加速度计量程 ±2G
    IMU660RA_ACC_SAMPLE_SGN_4G,  // 加速度计量程 ±4G
    IMU660RA_ACC_SAMPLE_SGN_8G,  // 加速度计量程 ±8G
    IMU660RA_ACC_SAMPLE_SGN_16G, // 加速度计量程 ±16G
} imu660ra_acc_sample_config;

typedef enum
{
    IMU660RA_GYRO_SAMPLE_SGN_125DPS,  // 陀螺仪量程 ±125DPS
    IMU660RA_GYRO_SAMPLE_SGN_250DPS,  // 陀螺仪量程 ±250DPS
    IMU660RA_GYRO_SAMPLE_SGN_500DPS,  // 陀螺仪量程 ±500DPS
    IMU660RA_GYRO_SAMPLE_SGN_1000DPS, // 陀螺仪量程 ±1000DPS
    IMU660RA_GYRO_SAMPLE_SGN_2000DPS, // 陀螺仪量程 ±2000DPS
} imu660ra_gyro_sample_config;

#define IMU660RA_ACC_SAMPLE_DEFAULT (IMU660RA_ACC_SAMPLE_SGN_8G)        // 默认的加速度计量程设置
#define IMU660RA_GYRO_SAMPLE_DEFAULT (IMU660RA_GYRO_SAMPLE_SGN_2000DPS) // 默认的陀螺仪量程设置
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     将 IMU660RA 加速度计数据转换为物理量
// 参数说明     acc_value       任意轴的加速度计原始数据
// 返回参数     void
// 使用示例     float data = imu660ra_acc_transition(imu660ra_acc_x);           // 单位 g(m/s^2)
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
#define imu660ra_acc_transition(acc_value) ((float)(acc_value) / imu660ra_transition_factor[0])

//-------------------------------------------------------------------------------------------------------------------
// 函数简介     将 IMU660RA 陀螺仪数据转换为物理量
// 参数说明     gyro_value      任意轴的陀螺仪原始数据
// 返回参数     void
// 使用示例     float data = imu660ra_gyro_transition(imu660ra_gyro_x);         // 单位 °/s
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
