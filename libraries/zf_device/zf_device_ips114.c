 ...
 *
 * AI8051U 开源库 为免费软件
 * 您可以根据自由软件基金会发布的 GPL（GNU General Public License）条款
 * 即 GPL3.0 或（您选择的）任何后来的版本，重新发布和/或修改它
 *
 * 本开源库的发布是希望它能发挥作用，但并未对其作任何保证
 * 甚至没有隐含的适销性或适合特定用途的保证
 * 更多细节请参见 GPL
 * 许可证副本位于 libraries 目录下的 LICENSE 文件
 *
 * 文件名称        zf_device_ips114.c
 * 公司名称        成都逐飞科技有限公司
 * 版本信息        查看 libraries/doc 目录内 version.txt
 * 开发环境        MDK FOR C251
 * 适用平台        AI8051U
...
// @return      void
// Sample usage:                ips114_show_int32(0,0,x,3);                     // x 可以为 int32 uint16 int16 uint8 int8 类型
...
void ips114_show_int32(uint16 x, uint16 y, int32 dat, uint8 num)
{
    ...
    num++;
    if (0 > dat)
        length = zf_sprintf(&buff[0], (const int8 *)"%d", dat); // 负数
    else
    {
        buff[0] = ' ';
...
// @return      void
// Sample usage:                ips114_show_float(0,0,x,2,3);                    // 显示浮点数   整数显示2位   小数显示三位
