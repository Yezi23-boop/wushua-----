...
uint32 wireless_uart_send_buffer (const uint8 *buff, uint32 len)
{
    ...
    while(0 != len)
    {
        if(!gpio_get_level(WIRELESS_UART_RTS_PIN))                              // 如果RTS为低电平 则继续发送数据
        {
            ...
            {
                uart_write_buffer(WIRELESS_UART_INDEX, buff, (uint16)len);              // 发送数据
                len = 0;
                break;
            }
        }
...
uint32 wireless_uart_send_string (const char *str)
{
    ...
    while(0 != len)
    {
        if(!gpio_get_level(WIRELESS_UART_RTS_PIN))                              // 如果RTS为低电平 则继续发送数据
        {
            ...
            {
				memcpy(temp, str, 30);
                uart_write_buffer(WIRELESS_UART_INDEX, temp, 30); // 发送数据
                str += 30;                                                      // 地址偏移
                len -= 30;                                                      // 数量
                time_count = 0;
            }
            else                                                                // 不足 30byte 的数据一次性发送完毕
            {
                ...
                uart_write_buffer(WIRELESS_UART_INDEX, temp, (uint16)len);// 发送数据
                len = 0;
                break;
            }
        }
