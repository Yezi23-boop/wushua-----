...
static const EepromConfigItem eeprom_config_items[] = {
    ...
    EEPROM_FLOAT(speed.kp2_Err, 19, 0.024f),      // 转向环二次项非线性增强系数
    /* --- MODEL 页面 --- */
    EEPROM_FLOAT(angle.kp_Angle, 20, 1.44f),       // 角速度内环比例系数Kp
    EEPROM_FLOAT(angle.kd_Angle, 21, 1.15f),       // 角速度内环微分系数Kd
    EEPROM_FLOAT(angle.limiting_Angle, 22, 58.0f), // 角速度内环输出限幅
    ...
    /* --- RING ENTRY 子页面 --- */
    EEPROM_FLOAT(ring.small_profile.bias_entry_gain, 28, 2.50f),       // 小圆环进环阶段同侧两路电感放大倍数
    EEPROM_FLOAT(ring.small_profile.bias_exit_gain, 29, 1.50f),        // 小圆环出环阶段对侧两路电感放大倍数
    ...
    /* --- RING DRIVE 子页面 --- */
    EEPROM_FLOAT(ring.small_profile.kp_Angle, 41, 1.44f), // 小圆环阶段角速度内环比例系数
    EEPROM_FLOAT(ring.small_profile.kd_Angle, 42, 1.15f), // 小圆环阶段角速度内环微分系数
    ...
    EEPROM_FLOAT(ring.large_profile.bias_exit_gain, 46, 1.50f),        // 大圆环出环阶段对侧两路电感放大倍数
    ...
    EEPROM_FLOAT(ring.large_profile.kp_Angle, 58, 1.44f), // 大圆环阶段角速度内环比例系数
    EEPROM_FLOAT(ring.large_profile.kd_Angle, 59, 1.15f), // 大圆环阶段角速度内环微分系数
    /* --- CYLINDER 页面 --- */
    ...
    EEPROM_INT(fly.seesaw_wait_count, 79, 10),       // 停止等待模式停车等待时间，×2ms
