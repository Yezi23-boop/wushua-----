...
static float calculate_forward_preference(float base_duty, float vzc, float vxc, float vyc)
{
    ...
    if (strength < 0.1f) return base_duty;
...
{
    float ratio, delta;
    DUTY_BOTTOM = g_app_config.start.fuya_xili;

    switch (p)
    {
    case 0: return DUTY_BOTTOM;
    case 1: /* 指数型过渡：平地 -> 左墙 */
        ratio = 1.0f - vzc;
...
void fuya_update_simple(void)
{
    ...
    switch (phase)
    {
        case 0: alpha = ALPHA_UP1;   break;
        case 1: alpha = ALPHA_UP2;   break;
        case 2: alpha = ALPHA_DOWN1; break;
        case 3: alpha = ALPHA_DOWN2; break;
        default: alpha = ALPHA_DEFAULT; break;
    }
    ...
    fuya_motor_output((int)g_app_config.start.fuya_xili);
}
